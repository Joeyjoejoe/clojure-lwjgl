...
(defn setup [path]
  (let [tbo-id (GL11/glGenTextures)
	...
	texture-data (STBImage/stbi_load path width height color desired-color-channels)]
    (when (nil? texture-data) (throw (Exception. (str "Texture loading failed: " (STBImage/stbi_failure_reason) " at " path))))
